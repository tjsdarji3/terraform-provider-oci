...
func init() {
	RegisterDatasource("oci_core_network_security_group_vnics", CoreNetworkSecurityGroupVnicsDataSource())
}
...
func (s *CoreNetworkSecurityGroupVnicsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreNetworkSecurityGroupVnicsDataSource().Schema["network_security_group_vnics"].Elem.(*schema.Resource).Schema)
	}

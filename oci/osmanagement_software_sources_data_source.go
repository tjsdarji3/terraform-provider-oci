	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"software_sources": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(OsmanagementSoftwareSourceResource()),
			},
		},
	}
}
...
func (s *OsmanagementSoftwareSourcesDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			softwareSource["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, OsmanagementSoftwareSourcesDataSource().Schema["software_sources"].Elem.(*schema.Resource).Schema)
	}

...
func init() {
	RegisterDatasource("oci_core_cluster_network_instances", CoreClusterNetworkInstancesDataSource())
}
...
func (s *CoreClusterNetworkInstancesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreClusterNetworkInstancesDataSource().Schema["instances"].Elem.(*schema.Resource).Schema)
	}

...
func init() {
	RegisterDatasource("oci_database_autonomous_exadata_infrastructures", DatabaseAutonomousExadataInfrastructuresDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"autonomous_exadata_infrastructures": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseAutonomousExadataInfrastructureResource()),
			},
		},
	}
}
...
func (s *DatabaseAutonomousExadataInfrastructuresDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			autonomousExadataInfrastructure["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseAutonomousExadataInfrastructuresDataSource().Schema["autonomous_exadata_infrastructures"].Elem.(*schema.Resource).Schema)
	}

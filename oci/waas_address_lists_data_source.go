	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"address_lists": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     GetDataSourceItemSchema(WaasAddressListResource()),
			},
		},
	}
}
...
func (s *WaasAddressListsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			addressList["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, WaasAddressListsDataSource().Schema["address_lists"].Elem.(*schema.Resource).Schema)
	}

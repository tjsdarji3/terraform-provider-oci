...
func init() {
	RegisterDatasource("oci_health_checks_vantage_points", HealthChecksVantagePointsDataSource())
}
...
func (s *HealthChecksVantagePointsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, HealthChecksVantagePointsDataSource().Schema["health_checks_vantage_points"].Elem.(*schema.Resource).Schema)
	}

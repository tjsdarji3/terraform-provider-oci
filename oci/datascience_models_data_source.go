...
func init() {
	RegisterDatasource("oci_datascience_models", DatascienceModelsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"models": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatascienceModelResource()),
			},
		},
	}
}
...
func (s *DatascienceModelsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			model["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatascienceModelsDataSource().Schema["models"].Elem.(*schema.Resource).Schema)
	}

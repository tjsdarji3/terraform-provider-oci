...
func init() {
	RegisterDatasource("oci_core_network_security_groups", CoreNetworkSecurityGroupsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"network_security_groups": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(CoreNetworkSecurityGroupResource()),
			},
		},
	}
}
...
func (s *CoreNetworkSecurityGroupsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			networkSecurityGroup["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreNetworkSecurityGroupsDataSource().Schema["network_security_groups"].Elem.(*schema.Resource).Schema)
	}

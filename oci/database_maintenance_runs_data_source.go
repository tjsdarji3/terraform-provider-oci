...
func init() {
	RegisterDatasource("oci_database_maintenance_runs", DatabaseMaintenanceRunsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"maintenance_runs": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseMaintenanceRunResource()),
			},
		},
	}
}
...
func (s *DatabaseMaintenanceRunsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.TimeScheduled != nil {
			maintenanceRun["time_scheduled"] = r.TimeScheduled.String()
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseMaintenanceRunsDataSource().Schema["maintenance_runs"].Elem.(*schema.Resource).Schema)
	}

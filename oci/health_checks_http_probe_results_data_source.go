...
func init() {
	RegisterDatasource("oci_health_checks_http_probe_results", HealthChecksHttpProbeResultsDataSource())
}
...
func (s *HealthChecksHttpProbeResultsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, HealthChecksHttpProbeResultsDataSource().Schema["http_probe_results"].Elem.(*schema.Resource).Schema)
	}

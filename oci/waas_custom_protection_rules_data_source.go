	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"custom_protection_rules": {
				Type:     schema.TypeList,
				Computed: true,
				Elem:     GetDataSourceItemSchema(WaasCustomProtectionRuleResource()),
			},
		},
	}
}
...
func (s *WaasCustomProtectionRulesDataSourceCrud) Get() error {
	...
	if states, ok := s.D.GetOkExists("states"); ok {
		...
		tmp := make([]oci_waas.ListCustomProtectionRulesLifecycleStateEnum, len(interfaces))
		for i := range interfaces {
			if interfaces[i] != nil {
				tmp[i] = oci_waas.ListCustomProtectionRulesLifecycleStateEnum(interfaces[i].(string))
			}
		}
...
func (s *WaasCustomProtectionRulesDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			customProtectionRule["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, WaasCustomProtectionRulesDataSource().Schema["custom_protection_rules"].Elem.(*schema.Resource).Schema)
	}

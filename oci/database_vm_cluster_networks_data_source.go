...
func init() {
	RegisterDatasource("oci_database_vm_cluster_networks", DatabaseVmClusterNetworksDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vm_cluster_networks": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatabaseVmClusterNetworkResource()),
			},
		},
	}
}
...
func (s *DatabaseVmClusterNetworksDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			vmClusterNetwork["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseVmClusterNetworksDataSource().Schema["vm_cluster_networks"].Elem.(*schema.Resource).Schema)
	}

...
func init() {
	RegisterDatasource("oci_dataflow_run_logs", DataflowRunLogsDataSource())
}
...
func (s *DataflowRunLogsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataflowRunLogsDataSource().Schema["run_logs"].Elem.(*schema.Resource).Schema)
	}

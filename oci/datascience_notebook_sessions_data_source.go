...
func init() {
	RegisterDatasource("oci_datascience_notebook_sessions", DatascienceNotebookSessionsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"notebook_sessions": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DatascienceNotebookSessionResource()),
			},
		},
	}
}
...
func (s *DatascienceNotebookSessionsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			notebookSession["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatascienceNotebookSessionsDataSource().Schema["notebook_sessions"].Elem.(*schema.Resource).Schema)
	}

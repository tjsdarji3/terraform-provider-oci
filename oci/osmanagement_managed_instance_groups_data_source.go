...
func init() {
	RegisterDatasource("oci_osmanagement_managed_instance_groups", OsmanagementManagedInstanceGroupsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"managed_instance_groups": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(OsmanagementManagedInstanceGroupResource()),
			},
		},
	}
}
...
func (s *OsmanagementManagedInstanceGroupsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			managedInstanceGroup["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, OsmanagementManagedInstanceGroupsDataSource().Schema["managed_instance_groups"].Elem.(*schema.Resource).Schema)
	}

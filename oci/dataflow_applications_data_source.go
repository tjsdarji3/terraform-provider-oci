...
func init() {
	RegisterDatasource("oci_dataflow_applications", DataflowApplicationsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"applications": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DataflowApplicationResource()),
			},
		},
	}
}
...
func (s *DataflowApplicationsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataflowApplicationsDataSource().Schema["applications"].Elem.(*schema.Resource).Schema)
	}

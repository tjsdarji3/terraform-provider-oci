...
func init() {
	RegisterDatasource("oci_resourcemanager_stacks", ResourcemanagerStacksDataSource())
}
...
func (s *ResourcemanagerStacksDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			stack["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, ResourcemanagerStacksDataSource().Schema["stacks"].Elem.(*schema.Resource).Schema)
	}

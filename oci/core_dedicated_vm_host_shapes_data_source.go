...
func init() {
	RegisterDatasource("oci_core_dedicated_vm_host_shapes", CoreDedicatedVmHostShapesDataSource())
}
...
func (s *CoreDedicatedVmHostShapesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, CoreDedicatedVmHostShapesDataSource().Schema["dedicated_vm_host_shapes"].Elem.(*schema.Resource).Schema)
	}

...
func init() {
	RegisterDatasource("oci_functions_functions", FunctionsFunctionsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"functions": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(FunctionsFunctionResource()),
			},
		},
	}
}
...
func (s *FunctionsFunctionsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			function["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, FunctionsFunctionsDataSource().Schema["functions"].Elem.(*schema.Resource).Schema)
	}

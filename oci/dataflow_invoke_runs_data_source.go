...
func init() {
	RegisterDatasource("oci_dataflow_invoke_runs", DataflowInvokeRunsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"runs": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(DataflowInvokeRunResource()),
			},
		},
	}
}
...
func (s *DataflowInvokeRunsDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			invokeRun["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DataflowInvokeRunsDataSource().Schema["runs"].Elem.(*schema.Resource).Schema)
	}

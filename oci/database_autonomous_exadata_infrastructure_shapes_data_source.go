...
func init() {
	RegisterDatasource("oci_database_autonomous_exadata_infrastructure_shapes", DatabaseAutonomousExadataInfrastructureShapesDataSource())
}
...
func (s *DatabaseAutonomousExadataInfrastructureShapesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, DatabaseAutonomousExadataInfrastructureShapesDataSource().Schema["autonomous_exadata_infrastructure_shapes"].Elem.(*schema.Resource).Schema)
	}

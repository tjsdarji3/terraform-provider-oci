...
func init() {
	RegisterDatasource("oci_limits_limit_values", LimitsLimitValuesDataSource())
}
...
func (s *LimitsLimitValuesDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, LimitsLimitValuesDataSource().Schema["limit_values"].Elem.(*schema.Resource).Schema)
	}

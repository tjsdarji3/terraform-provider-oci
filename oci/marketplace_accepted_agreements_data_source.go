...
func init() {
	RegisterDatasource("oci_marketplace_accepted_agreements", MarketplaceAcceptedAgreementsDataSource())
}
	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"accepted_agreements": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(MarketplaceAcceptedAgreementResource()),
			},
		},
	}
}
...
func (s *MarketplaceAcceptedAgreementsDataSourceCrud) SetData() error {
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, MarketplaceAcceptedAgreementsDataSource().Schema["accepted_agreements"].Elem.(*schema.Resource).Schema)
	}

	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"oda_instances": {
				...
				Computed: true,
				Elem:     GetDataSourceItemSchema(OdaOdaInstanceResource()),
			},
		},
	}
}
...
func (s *OdaOdaInstancesDataSourceCrud) SetData() error {
	...
	for _, r := range s.Res.Items {
		...
		if r.DefinedTags != nil {
			odaInstance["defined_tags"] = definedTagsToMap(r.DefinedTags)
		}
	...
	if f, fOk := s.D.GetOkExists("filter"); fOk {
		resources = ApplyFilters(f.(*schema.Set), resources, OdaOdaInstancesDataSource().Schema["oda_instances"].Elem.(*schema.Resource).Schema)
	}
